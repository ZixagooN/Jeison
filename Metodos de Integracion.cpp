...
for(int N=1; N<=10000; ++N){
    ...
    datafile<<N<<" "<<integral<<" "<<integral_simpson<<" "<<integral_exacta<<std::endl;
    }
datafile.close();



std::ofstream scriptFile1("grafico_integral_numerica_metodos.gp");
scriptFile1<<"set term png\n";
scriptFile1<<"set output 'grafico_integral_numerica_metodos.png'\n";
scriptFile1<<"set xlabel 'N'\n";
scriptFile1<<"set ylabel 'Integral'\n";
scriptFile1<<"set logscale x\n";
scriptFile1<<"plot 'resultados_cuadratura_rectangular.dat' u 1:2 w l title 'Integral aproximada', '' u 1:3 w l title 'Integral Simpson', '' u 1:4 w l title 'Valor exacto'\n";
scriptFile1.close();
